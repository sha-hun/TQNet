...
class LearnableHalfCyclePrior(nn.Module):
    def __init__(self, T: int, init_beta=0.05, init_alpha=1.0, init_width=4.0, center_mean=False):
        super().__init__()
        self.T = int(T)
        self.center_mean = bool(center_mean)

        # 用 softplus 参数化保证正数
        self._beta  = nn.Parameter(torch.tensor(float(init_beta)).log().exp().log())   # 仅占位，下面用 softplus 取正
        self._alpha = nn.Parameter(torch.tensor(float(init_alpha)).log().exp().log())
        self._width = nn.Parameter(torch.tensor(float(init_width)).log().exp().log())

        idx = torch.arange(self.T)
        dist = (idx[:, None] - idx[None, :]).abs()
        diff = torch.minimum(dist, self.T - dist).float()
        self.register_buffer("diff", diff, persistent=False)  # (T,T)

    def forward(self):
        beta  = F.softplus(self._beta)
        alpha = F.softplus(self._alpha)
        width = F.softplus(self._width)

        center = 0.5 * self.T
        bell = -alpha * torch.exp(-((self.diff - center) / width) ** 2)
        exp_decay = -beta * self.diff

        log_prior = exp_decay + bell
        log_prior = log_prior - log_prior.max()  # 作为 additive bias 更稳定
        if self.center_mean:
            log_prior = log_prior - log_prior.mean()
        return log_prior  # (T,T)
...
class HybridDecomposition(nn.Module):
    def __init__(self,config):
        ...
        self.pre_log_dis = torch.log(
            1 / (1 + torch.exp(0.5 * (self.diff - seq_len / 2))) +
            torch.exp(-self.diff) / (1 + torch.exp(torch.tensor(seq_len / 4.0, device=self.diff.device)))
        )
        ...
        self.learnable_prior = LearnableHalfCyclePrior(T=seq_len, init_beta=0.05, init_alpha=1.0, init_width=4.0, center_mean=False)
    ...
    def forward(self, X_Residual, mask, time_dif, time_idx):
        ...
        log_func = self.learnable_prior()  # (T, T)
        # 处理 H_idx
        t_i = time_idx.float()  # (B, T, 1)
        A_idx = torch.exp(-abs(t_i - t_i.transpose(1, 2)) / tau_idx) + log_func#self.pre_log_dis.to(t_i.device)   # (B, T, T)
        ...
        A_dif = torch.exp(-abs(t_d_j - t_d_i) / tau_dif) + log_func.unsqueeze(-1)#self.pre_log_dis.unsqueeze(-1).to(t_d.device) # (B, T, T, C)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.first_print_shape = True



    def forward(self, x_enc, cycle_index,**kwargs):  # (batch_x, batch_x_mark, dec_inp, batch_y_mark,batch_y,batch_cycle)
        
        x = x_enc[:, :, :self.C]  # (B, T, C)
        mask = x_enc[:, :, self.C:2*self.C]  # (B, T, C)
        time_dif = x_enc[:, :, 2*self.C:3*self.C]  # (B, T, C)
        time_idx = x_enc[:, :, 3*self.C:3*self.C+1] # (B, T, 1)
        if self.first_print_shape:
            print("输入x_enc的形状:", x_enc.shape)
            print("输入x的形状:", x.shape)
            print("输入mask的形状:", mask.shape)
            print("输入time_dif的形状:", time_dif.shape)
            print("输入time_idx的形状:", time_idx.shape)
            self.first_print_shape = False

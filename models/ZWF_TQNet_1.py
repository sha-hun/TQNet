...
def neg_gaussian_bell_TT(T: int, width: float, device=None, dtype=torch.float32, center_mean: bool = False):
    """
    y(d) = -exp(-((d - T/2)/width)^2)  with d = circular distance in [0, T/2]
    返回: bell (T,T)
    """
    idx = torch.arange(T, device=device)
    dist = (idx[:, None] - idx[None, :]).abs()
    diff = torch.minimum(dist, T - dist).to(dtype)  # (T,T)

    center = float(T) / 2.0
    bell = -torch.exp(-((diff - center) / float(width)) ** 2)
...
class HybridDecomposition(nn.Module):
    ...
    def forward(self, X_Residual, mask, time_dif, time_idx):
        ...
        log_func = neg_gaussian_bell_TT(T, width=4.0, device=X_Residual.device)
        # 处理 H_idx

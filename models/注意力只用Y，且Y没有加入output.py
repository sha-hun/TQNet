...
class Model(nn.Module):
    ...
    def forward(self, x, cycle_index):
        ...
        # 这里将key跟value都改成Y_seasonal，看看效果
        
        channel_information = self.channelAggregator(query=Y_seasonal, key=Y_seasonal, value=Y_seasonal)[0]
        input = self.input_proj(query_input + channel_information)

        hidden = self.model(input)
        # output = self.output_proj(hidden+input).permute(0, 2, 1) + self.seasonal_weight * Y_seasonal
        output = self.output_proj(hidden+input).permute(0, 2, 1)
        # self.logger.info("Y_seasonal mean abs: {:.4f}, output mean abs: {:.4f}".format(
        #     Y_seasonal.abs().mean().item(),
